...
yes|cp -r deploy/manifest.yml deploy/manifest-dev.yml
sed -i "s/  domain: mybluemix.net/  domain: $domainname/g" deploy/manifest-dev.yml
sed -i "s/  host: ipm-marketplace-app-stage/  host: $temp_inst/g" deploy/manifest-dev.yml
sed -i "s/- name: ipm-marketplace-app-stage/- name: $temp_inst/g" deploy/manifest-dev.yml
sed -i "s/  instances: 1/  instances: $number_of_instances/g" deploy/manifest-dev.yml
...
cat env.txt >> deploy/manifest-dev.yml
...
if [ $errorCode == 0 ]; then
        ...
        ibmcloud cf push -f deploy/manifest-dev.yml
        errorCode=$?
        if [ $errorCode != 0 ]; then
                echo " Command \"ibmcloud cf push -f deploy/manifest-dev.yml\" failed for the new $temp_inst app."
                exit $errorCode
        fi
        ...
        echo "Blue-green deployment for ipm-marketplace-app-dev app successful."
else
